...
class Hand:
    ...
    def __init__(self, cards: list[Card]) -> None:
        ...
        self.poker_hand = -1
        self.scored_cards = []
        self.unscored_cards = sorted_cards
        for i, poker_hand in enumerate(POKER_HANDS):
            scored_cards, unscored_cards = poker_hand(sorted_cards)
            if scored_cards != []:
                self.poker_hand = len(POKER_HANDS) - i
    ...
    def greater_than_hand(self, other: "Hand") -> bool | None:
        ...
        if self.poker_hand in {4, 9}: #Ace low straights special tie break
            return self.scored_cards[-2] > other.scored_cards[-2] # Compare the highest non-ace cards' suits
        if self.poker_hand == 7: #Full House special tie break
...
def four_ofa_kind(cards: list[Card]) -> tuple[list[Card], list[Card]]:
    ...
    unscored_cards = list(cards)
    ranks = [card.rank for card in unscored_cards]
    ranks_set = set(ranks)
...
def straight_flush(cards: list[Card]) -> tuple[list[Card], list[Card]]:
    ...
    print(f"Checking straight_flush, cards: {[str(card) for card in cards]}")
    suits = [card.suit for card in cards]
    print(f"suits: {suits}")
    if suits.count(suits[0]) == 5:
        low_card_rank = cards[0].rank
...
POKER_HANDS = (straight_flush, ace_low_straight_flush, four_ofa_kind, full_house, flush, straight, ace_low_straight, three_ofa_kind, two_pair, pair)
