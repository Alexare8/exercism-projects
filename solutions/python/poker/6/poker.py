...
class Hand:
    ...
    def __init__(self, cards: list[Card]) -> None:
        ...
        self.suits = Counter([card.suit for card in self.unscored_cards])
    ...
    def greater_than_hand(self, other: "Hand") -> bool | None:
        ...
        if self.poker_hand == 6: #Full House special tie break
            return self.ranks.most_common(1) > other.ranks.most_common(1)
...
def common_rank(cards: list[Card], ranks: Counter, rank_count: int) -> tuple[list, list]:
...
def pair(cards: list[Card], ranks: Counter, _: Counter) -> tuple[list[Card], list[Card]]:
    """Check if a hand is a pair."""
    return common_rank(cards, ranks, 2)


def two_pair(cards: list[Card], ranks: Counter, _: Counter) -> tuple[list[Card], list[Card]]:
    """Check if a hand is two pairs."""
    pair, remainder = common_rank(cards, ranks, 2)
...
def three_ofa_kind(cards: list[Card], ranks: Counter, _: Counter) -> tuple[list[Card], list[Card]]:
    """Check if a hand is a three of a kind."""
    return common_rank(cards, ranks, 3)


def ace_low_straight(cards: list[Card], ranks: Counter, _: Counter) -> tuple[list[Card], list[Card]]:
...
def straight(cards: list[Card], ranks: Counter, _: Counter) -> tuple[list[Card], list[Card]]:
...
def flush(cards: list[Card], _: Counter, suits: Counter) -> tuple[list[Card], list[Card]]:
...
def full_house(cards: list[Card], ranks: Counter, _: Counter) -> tuple[list[Card], list[Card]]:
    """Check if hand is a full house."""
    if len(ranks) == 2 and ranks.most_common(1)[0][1] in {2, 3}:
        return (cards, [])
    return ([], cards)


def four_ofa_kind(cards: list[Card], ranks: Counter, _: Counter) -> tuple[list[Card], list[Card]]:
    """Check if hand is a four of a kind."""
    return common_rank(cards, ranks, 4)


def ace_low_straight_flush(cards: list[Card], ranks: Counter, suits: Counter) -> tuple[list[Card], list[Card]]:
...
def straight_flush(cards: list[Card], ranks: Counter, suits: Counter) -> tuple[list[Card], list[Card]]:
    """Check if hand is a straight flush."""
    low_card_rank = cards[0].rank
    rank_series = {rank - low_card_rank for rank in ranks}
    if rank_series == {0, 1, 2, 3, 4} and len(suits) == 1:
        return (cards, [])
    return ([], cards)
