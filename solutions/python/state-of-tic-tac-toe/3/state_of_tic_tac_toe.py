"""
state_of_tic_tac_toe - Evaluate the state of a Tic Tac Toe game board.

Accepts game boards as lists of lists of "X" and "O".
"""

def gamestate(board) -> str:
    """Evaluate the game state as win, draw, or onging."""
    flat_board = "".join(board)
    total_x = "".join(flat_board).count("X")
    total_o = "".join(flat_board).count("O")
...
def detect_wins(board) -> tuple[bool, bool]:
