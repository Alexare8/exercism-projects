...
def send_to_store(cart: dict[str, int], aisle_mapping: dict[str, list]) -> dict[str, list]:
    ...
    fulfillment_cart = {item: [quantity, *aisle_mapping[item]] for item, quantity in sorted(cart.items(), reverse=True)}
    return dict(fulfillment_cart.items())


def update_store_inventory(fulfillment_cart: dict[str, list], store_inventory: dict[str, list]) -> dict[str, list]:
    """Update store inventory levels with user order"""
    return store_inventory | {item: [store_inventory[item][0] - quantity or 'Out of Stock', aisle, refrig] for item, (quantity, aisle, refrig) in fulfillment_cart.items()}
