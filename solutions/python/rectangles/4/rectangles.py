    ...
    for top_left in corners[:-1]:
        for bottom_right in corners[1:]:
            if top_left[0] < bottom_right[0] and top_left[1] < bottom_right[1]:
                ...
                if all(corner in corners for corner in [top_left, top_right, bottom_left, bottom_right]):
                    if (
                        check_row(grid, top_left, top_right)
...
def check_row(grid: list[str], left: tuple[int, int], right: tuple[int, int]) -> bool:
    ...
    row = left[0]
    for cell in grid[row][left[1] + 1: right[1]]:
        if cell not in {"-", "+"}:
            return False
    return True
...
def check_side(grid: list[str], top: tuple[int, int], bottom: tuple[int, int]) -> bool:
    ...
    for cell in trasposed_grid[column][top[0] + 1: bottom[0]]:
        if cell not in {"|", "+"}:
            return False
    return True
